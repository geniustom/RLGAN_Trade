...
def CreateAgent(model,nb_actions,weight_path=None):
	...
	agent.compile(Adam(lr=3e-4), metrics=['mae'])
...
def Train(agent,env,nb_steps,weight_path=None):
	...
	agent.fit(env, nb_steps=nb_steps, callbacks=callbacks_list, visualize=False, verbose=1)
...
def BuildDDQNModel(env,nb_actions):
	...
	model.add(Reshape((8, 300), input_shape=(1,) +env.observation_space.shape))
	model.add(BatchNormalization())
	model.add(Conv1D(128, (3), strides=1, padding='valid', activation='relu'))
	model.add(Conv1D(128, (3), strides=1, padding='valid', activation='relu'))
	model.add(Conv1D(128, (3), strides=1, padding='valid', activation='relu'))
	model.add(Flatten())
	model.add(Dense(512))
