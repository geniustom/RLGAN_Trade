...
def runprocess(thread_id, s_t):
	...
	if GameTread[thread_id].DateIndex == 0 and lose!=0 and win!=0:
		...
		print("run:",cnt,"date:",GameTread[thread_id].Trade.date ,"profit:",points)
		lose=0
		win=0
		points=0
		profit=[]	
...
model = buildmodel()
#model = load_model("saved_models/model_updates_2000", custom_objects={'logloss': logloss, 'sumofsquares': sumofsquares})
